...
{
	rootProperty      = new rviz::Property("PanelRootProperty");
	topicsProperty    = new rviz::Property("Topics", QVariant(), QString(), rootProperty);
	simTopicProperty  = new rviz::RosTopicProperty("Simulation Topic", "/suturo/create_object", "suturo_action_tester/SuturoCreateObject", "Topic on which to connect to the simulation", topicsProperty, SLOT(updateTopic()), this);
	saveTopicProperty = new rviz::RosTopicProperty("Simulation Save Topic", "/suturo/save_scene", "std_msgs/String", "Topic on which to connect to the simulation", topicsProperty, SLOT(updateTopic()), this);
	loadTopicProperty = new rviz::RosTopicProperty("Simulation Load Topic", "/suturo/load_scene", "std_msgs/String", "Topic on which to connect to the simulation", topicsProperty, SLOT(updateTopic()), this);
	objRootProperty   = new rviz::Property("Object", QVariant(), QString(), rootProperty);
	objNameProperty   = new rviz::StringProperty("Object Name", "object1", "Name of the object to create", objRootProperty, SLOT(updateVisMarker()), this);
	objColorProperty  = new rviz::ColorProperty("Color", QColor(204,51,204), "Color of the marker", objRootProperty, SLOT(updateVisMarker()), this);
	objAlphaProperty  = new rviz::FloatProperty("Alpha", 1.0f, "Alpha of the marker", objRootProperty, SLOT(updateVisMarker()), this);
	objShapeProperty  = new rviz::EnumProperty("Shape", "Cube", "Shape of the new object", objRootProperty, SLOT(updateVisMarker()), this);
	objScaleProperty  = new rviz::VectorProperty("Scale", Ogre::Vector3(1,1,1), "Scale of the object", objRootProperty, SLOT(updateVisMarker()), this);
	objMeshProperty   = new rviz::StringProperty("Mesh", "", "Mesh for the object. First part of path is ros-package", objRootProperty, SLOT(updateVisMarker()), this);
	scenePathProperty = new rviz::StringProperty("Scene Path", "", "Path to save the scene to and load it from", rootProperty);
